...
int Fun4All_G4_Prototype4(int nEvents = 1)
{
  ...
  double tilted = 0;
  double tiltedrad = tilted/180*3.1415926538;
  ...
  gen->set_vertex_distribution_mean(0.0, 0.0, 3.5);
  gen->set_vertex_distribution_width(0.0, 8, 8);  // Rough beam profile size @ 16 GeV measured by Abhisek
  ...
  gen->set_phi_range(tiltedrad-0.001, tiltedrad+0.001);             // 1mrad angular divergence
  const double momentum = 8;
...
//    cemc->set_int_param("construction_verbose", 2);
	...
	cemc->set_double_param("z_rotation_degree", 5); // rotation around CG
  ...
  //  cemc_cells->get_light_collection_model().load_data_file(string(getenv("CALIBRATIONROOT")) + string("/CEMC/LightCollection/Prototype2Module.xml"), "data_grid_light_guide_efficiency", "data_grid_fiber_trans");
	cemc_cells->get_light_collection_model().load_data_file(string(getenv("CALIBRATIONROOT")) + string("/CEMC/LightCollection/Prototype3Module.xml"), "data_grid_light_guide_efficiency", "data_grid_fiber_trans");
