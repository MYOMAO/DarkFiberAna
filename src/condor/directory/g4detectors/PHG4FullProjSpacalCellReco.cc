...
#include <TCanvas.h>                           // for Plotting
#include <TROOT.h>
#include <TStyle.h>
...
//Declear Histograms and Canvas
TH2D * hisXY = new TH2D("HisXY","",106,-5,100,106,-5,100);
	...
	light_collection_model()
{
	...
	//Create TH2D to plot the XY ID to select the boundaries//
	...
	for(int i = 0; i < NBlocksX; i++){
		...
		for(int j = 0; j < NBlocksX; j++){
			...
			BlockIDs->SetBinContent(i+1,j+1,MaskIndex[i + j * 4]);
	...
	*/
	...
	hisXY->GetXaxis()->SetTitle("fiber_x");
	hisXY->GetYaxis()->SetTitle("fiber_y");
...
PHG4FullProjSpacalCellReco::InitRun(PHCompositeNode *topNode)
{
	...
	std::cout << "BRO: Modified this PHG4 CODES" << std::endl;
...
PHG4FullProjSpacalCellReco::process_event(PHCompositeNode *topNode)
{
	...
	for (hiter = hit_begin_end.first; hiter != hit_begin_end.second; ++hiter)
	{
		...
		//  std::cout << "BLOCKINFO:    tower_ID_z = " << tower_ID_z << "   tower_ID_phi = " << tower_ID_phi << std::endl;

		//	  BlockIDs->Fill(tower_ID_phi,tower_ID_z);
		...
		else
		{
			...
			//cout << "sub_tower_ID_x = " << sub_tower_ID_x << "    sub_tower_ID_y = " << sub_tower_ID_y << endl;
			//hisXY->Fill(sub_tower_ID_x,sub_tower_ID_y);
		...
		//light_yield = 0; //Masking all the fibers

		const int NFiberY = it_tower->second.NFiberY;
		const int fiber_x = decoder.fiber_ID/ NFiberY;
		const int fiber_y = decoder.fiber_ID% NFiberY;
		//cout << "NFiberY = " << NFiberY << "    fiber_x = " << fiber_x << "   fiber_y =  " << fiber_y << endl;
		/*
		   if(fiber_x == SmallestX || fiber_x == LargestX || fiber_y == SmallestY || fiber_y == LargestY)
		   {
		   light_yield = 0;  

		   }else{
		   hisXY->Fill(fiber_x,fiber_y);
		   }
		   */
/*
		if(fiber_x == LargestX && MaskIndex[tower_ID_phi + 4 * tower_ID_z] == 1)
		{
			light_yield = 0;  
		}

		if(fiber_x == SmallestX &&  MaskIndex[tower_ID_phi + 4 * tower_ID_z] == 2)
		{
			light_yield = 0;  
		}

		if(fiber_y == SmallestY && MaskIndex[tower_ID_phi + 4 * tower_ID_z] == 3)
		{
			light_yield = 0;  
		}
		if( (fiber_x == LargestX  || fiber_y == SmallestY) && MaskIndex[tower_ID_phi + 4 * tower_ID_z] == 23)
		{
			light_yield = 0;  
		}
		if( (fiber_x == SmallestX  || fiber_x == LargestX) && MaskIndex[tower_ID_phi + 4 * tower_ID_z] == 12)
		{
			light_yield = 0;  
		}
		if( (fiber_x == SmallestX  || fiber_y == SmallestY) && MaskIndex[tower_ID_phi + 4 * tower_ID_z] == 13)
		{
			light_yield = 0;  
		}
		if( (fiber_x == SmallestX  || fiber_x == LargestX || fiber_y == SmallestY) && MaskIndex[tower_ID_phi + 4 * tower_ID_z] == 123)
		{
			light_yield = 0;  
		}
*/
		...
		//Left = 1//
		if(fiber_y == SmallestY  && MaskIndex[tower_ID_phi + 4 * tower_ID_z] == 1)
		{
			light_yield = 0;  
		}
		...
		if(fiber_y == LargestY  && MaskIndex[tower_ID_phi + 4 * tower_ID_z] == 2)
		{
			light_yield = 0;  
		}

		//Down//
		if(fiber_x == SmallestX  && MaskIndex[tower_ID_phi + 4 * tower_ID_z] == 3)
		{
			light_yield = 0;  
		}


		if( ( fiber_y == LargestY  || fiber_x == SmallestX  ) && MaskIndex[tower_ID_phi + 4 * tower_ID_z] == 23)
		{
			light_yield = 0;  
		}
		if( (fiber_y == SmallestY ||fiber_y == LargestY  ) && MaskIndex[tower_ID_phi + 4 * tower_ID_z] == 12)
		{
			light_yield = 0;  
		}
		if( (fiber_y == SmallestY || fiber_x == SmallestX  ) && MaskIndex[tower_ID_phi + 4 * tower_ID_z] == 13)
		{
			light_yield = 0;  
		}
		if( (fiber_y == SmallestY   || fiber_y == LargestY  ||fiber_x == SmallestX  ) && MaskIndex[tower_ID_phi + 4 * tower_ID_z] == 123)
		{
			light_yield = 0;  
		}
	...
	//Draw The XY ID Plots//


	//    BlockIDs->Draw("COLZ");
	//	c->SaveAs("BLOCKIDPlot.png");
